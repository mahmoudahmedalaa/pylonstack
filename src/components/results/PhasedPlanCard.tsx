...
export function PhasedPlanCard({ phase }: { phase: ProjectPhase }) {
  ...
  const freeTools = phase.tools.filter(t => t.monthlyCost === 0 && t.pricingModel !== 'Usage-Based');
  const usageTools = phase.tools.filter(t => t.pricingModel === 'Usage-Based' && t.monthlyCost === 0);
    ...
    <div className="flex flex-col rounded-xl border border-[var(--border)] bg-[var(--card)]">
      ...
      <div className="border-b border-[var(--border)] px-5 py-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h3 className="text-lg font-semibold tracking-tight text-[var(--foreground)]">{phase.name}</h3>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 rounded-full bg-[var(--color-success-500)]/10 px-2.5 py-0.5 text-xs text-[var(--color-success-500)]">
              ...
              <span>
                {phase.estimatedMonthlyCost > 0 
                  ? `$${phase.estimatedMonthlyCost}/mo`
                  : '$0/mo'}
              </span>
            </div>
          </div>
        ...
        {phase.description && (
          <p className="mt-2 text-sm text-[var(--muted-foreground)] leading-relaxed">{phase.description}</p>
        )}
      </div>
      ...
      <div className="flex flex-col divide-y divide-[var(--border)]">
        {phase.tools.map((tool, idx) => {
           const catalogTool = TOOLS.find(t => t.id === tool.toolName.toLowerCase().replace(/[^a-z0-9]/g, '-') || t.name === tool.toolName);
           return (
            <div
              key={idx}
            ...
            >
              {catalogTool?.logo ? (
                <Image src={catalogTool.logo} alt={tool.toolName} width={32} height={32} className="h-8 w-8 object-contain shrink-0 rounded-md bg-white p-1" />
              ) : (
              ...
              <div className="flex flex-1 flex-col min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-base text-[var(--foreground)]">{tool.toolName}</span>
                  <span className="text-[11px] font-medium text-[var(--muted-foreground)] bg-[var(--muted)] rounded-full px-2.5 py-0.5">{tool.tier}</span>
                </div>
                {tool.reason && (
                  <p className="text-sm text-[var(--muted-foreground)] line-clamp-1 leading-relaxed">
                    {tool.reason}
                  </p>
                )}
              </div>
        ...
        <div className="flex flex-col gap-1 text-xs">
          {freeTools.length > 0 && (
            <div className="flex justify-between text-[var(--muted-foreground)]">
              <span>{freeTools.length} free component{freeTools.length !== 1 ? 's' : ''}</span>
              <span>$0</span>
            </div>
          )}
          {usageTools.length > 0 && (
          ...
          {paidTools.length > 0 && (
            <div className="flex justify-between text-[var(--muted-foreground)]">
              <span>{paidTools.length} paid tool{paidTools.length !== 1 ? 's' : ''}</span>
              <span>${fixedSubtotal}/mo</span>
            </div>
          )}
          <div className="mt-1 flex justify-between border-t border-[var(--border)] pt-1.5 font-semibold text-sm text-[var(--foreground)]">
            <span>Subtotal</span>
